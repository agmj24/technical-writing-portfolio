# DITA Topic Types in Practice (Sample)

> This shows how the same subject — a design rule check — gets split across DITA's three core topic types: concept, task, and reference. Each one has a different job, and mixing them into one long topic is the most common structural mistake I see in documentation that hasn't gone through a structured-authoring pass yet.

## Why Split Content This Way at All

The short answer: reuse and maintenance. If "what is a design rule check" and "how do I run one" and "here's the full flag list" all live in the same page, you can't pull just the conceptual explanation into a different manual without dragging the procedure and reference table along with it. Split into three topic types, each piece can be assembled into different outputs — a full user guide, a quick-start card, an in-tool help panel — without rewriting anything.

Here's what that split actually looks like, both in the DITA markup and in what the reader sees.

---

## Concept Topic: What Is a Design Rule Check?

A concept topic answers "what is this and why would I care" — no steps, no exhaustive parameter list, just enough to orient someone before they act.
...
Notice there's no "step 1, step 2" here, and no full parameter table. That content belongs in the other two topic types — pulling it in here would make this topic impossible to reuse cleanly wherever just the concept is needed (like a glossary panel or a training deck).
...
A reference topic is lookup content — tables, parameter lists, syntax — the stuff you don't read start to finish, you scan for the one row you need.
...
None of these three topics is meant to stand alone forever — they get assembled through a DITA map into whatever output actually needs them:
...
That same `drc-overview.dita` concept topic could also get pulled into a completely different map — say, a "New User Orientation" guide — without touching a single word of it, because it never depended on the task or reference content to make sense on its own. That's really the whole point of splitting things this way. It's more setup work up front than just writing one long page, but it's what makes a documentation set survive years of product updates without turning into a mess of copy-pasted, slowly drifting duplicates.
